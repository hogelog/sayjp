// Style-Bert-VITS2 の g2p.py (LGPL-3.0) を参照し Rust へ移植。pyopenjtalk のフルコンテキスト
// ラベルから取っていた韻律記号を、jpreprocess の構造化フィールドへ対応付けている。
// LGPL 全文: licenses/Style-Bert-VITS2-g2p-LGPL-3.0.txt

use crate::engine::error::{Error, Result};
use crate::engine::mora::{MORA_KATA_TO_MORA_PHONEMES, VOWELS};
use crate::engine::norm::{replace_punctuation, PUNCTUATIONS};
...
fn initialize_jtalk() -> Result<JPreprocessType> {
    ...
    // agpl_dict (all.bin) は「降水確率」等の普通の語で品詞解析がクラッシュするため使わない。
...
static KATAKANA_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"[\u{30A0}-\u{30FF}]+").unwrap());
    ...
    fn fix_phone_tone(&self, phone_tone_list: Vec<(String, i32)>) -> Result<Vec<(String, i32)>> {
        ...
        if tone_values.len() == 1 {
            ...
            if tone_values == hash_set![0, 1] {
                ...
                Err(Error::Value("Invalid tone values 0".to_string()))
            }
        } else {
            Err(Error::Value("Invalid tone values 1".to_string()))
        }
    }
    ...
    fn align_tones(
        ...
        for phone in phone_with_punct.clone() {
            if tone_index >= phone_tone_list.len() {
                ...
                return Err(Error::Value(format!("Mismatched phoneme: {phone}")));
            }
        }
